  ...
  if (orderCount < N) {
    ...
    switch (order) {
      ...
      case 'recommend3':
        ...
        let value = -1;
        let copyQueue = new PriorityQueue(
          [...heapMap.get(G)[condition].copy()],
          X === 1 ? minHeapCondition : maxHeapCondition
        );
        // console.log(heapMap.get(G)[condition].copy());
        // console.log(copyQueue.queue);
        item = copyQueue.pop();
        while (true) {
          // console.log(item);
          if (solvedMap.get(item.p) !== item.l) {
